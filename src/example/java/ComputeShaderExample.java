...
public class ComputeShaderExample extends GLFWApplicationRunner {
    ...
    protected void init() {
        ...
        final MemoryBuffer counterMemoryBuffer = new MemoryBuffer();
        for (int i = 0; i < COUNT; i++) {
            counterMemoryBuffer.writeInt(i + 10);
        }
        final Buffer counterBuffer = new ImmutableBuffer(counterMemoryBuffer.finish(), GL44C.GL_CLIENT_STORAGE_BIT);
        counterMemoryBuffer.free();
        ...
        final MemoryBuffer counterResultMemoryBuffer = new MemoryBuffer(counterBuffer.download());
        for (int i = 0; i < counterResult.length; i++) {
            counterResult[i] = counterResultMemoryBuffer.readInt();
        }
        counterResultMemoryBuffer.free();
